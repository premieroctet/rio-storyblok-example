...
const REDIRECTIONIO_INSTANCE_NAME =
  process.env.REDIRECTIONIO_INSTANCE_NAME || "redirection-io-vercel-middleware";
const REDIRECTIONIO_VERSION = "redirection-io-vercel-middleware/0.3.5";
const REDIRECTIONIO_ADD_HEADER_RULE_IDS = process.env
...
export const createRedirectionIoMiddleware = (
  ...
  return async (request, context) => {
    ...
    if (config.previousMiddleware) {
      const response = await config.previousMiddleware(
        middlewareRequest,
        context
      );
      ...
      middlewareRequest = middlewareResponseToRequest(
        middlewareRequest,
        response,
        body
      );
...
async function handler(
  ...
  if (!REDIRECTIONIO_TOKEN) {
    console.warn(
      "No REDIRECTIONIO_TOKEN environment variable found. Skipping redirection.io middleware."
    );
  ...
  const [response, backendStatusCode] = await proxy(
    request,
    action,
    (request) => {
...
function middlewareResponseToRequest(
  ...
  if (response.headers.has("x-middleware-override-headers")) {
    const headersToOverride = response.headers.get(
...
async function fetchRedirectionIOAction(
...
async function proxy(
  ...
  try {
    ...
    const newHeaderMap = action.filter_headers(
      headerMap,
      backendStatusCode,
      REDIRECTIONIO_ADD_HEADER_RULE_IDS
    );
    ...
    for (let i = 0; i < newHeaderMap.len(); i++) {
      newHeaders.append(
        newHeaderMap.get_header_name(i),
        newHeaderMap.get_header_value(i)
      );
    }
    ...
    const bodyFilter = action.create_body_filter(
      backendStatusCode,
      newHeaderMap
    );
